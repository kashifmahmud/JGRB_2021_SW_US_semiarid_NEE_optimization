...
## ---------------------
# Set-up
rundir = '/Users/kashifmahmud/IU/Work/All_analyses/semi-arid_sites/output/for_plots/' # - CHANGE THIS
site_file = rundir+'siteinfo.txt' # - SET-UP THIS FILE FOR ALL SITES (SITE NAME, START YEAR, END YEAR)
param_file = rundir+'param_list.xlsx' # - SET-UP THIS FILE FOR ALL OPTIMIZED PARAMETER LIST
...
validation_name = ['nee_iter3840_paramP1_post','nee_iter3360_paramP2_post','nee_iter2880_paramP3_post',
              'nee_iter2400_paramP4_post','nee_iter2160_paramP5_post','nee_iter1680_paramP6_post','nee_iter720_paramP7_post']
...
pal = dict(Obs="gray", Prior="red", Post="blue")
col_wrap = 3
    
df_rmse_all = df_corr_all = df_corr_val_all = df_param_all = flux_corr_all = flux_slope_all = pd.DataFrame([])
flux_msd_all = flux_msd_all_month = flux_msd_all_annual = pd.DataFrame([])
df_annual_all = df_month_mean_all = df_rmse_val_all = pd.DataFrame([])
...
#    i=0
    ...
    val_root = rundir + '%s/%s_' + validation_name[i] + '/output.nc'
    ...
    df_flux = df_flux_val = df_rmse = df_rmse_site = df_anomaly = anomaly_site_all = df_month_mean = df_error = pd.DataFrame([])
    df_param = df_param_sub = df_msd = df_msd_month = df_msd_annual = df_month = df_annual = df_rmse_val = pd.DataFrame([])
    ...
    #    ns = 0
        ...
        flux_val = val_root.replace("%s", ss)
        
        ###### GPP and NEE time series plot
        # - load output nc file
        data = netCDF4.Dataset(flux, 'r')
        data_val = netCDF4.Dataset(flux_val, 'r')
...
#        years_df = time.to_datetimeindex()
        ...
        # get the time steps for each validation site
        NEE_val = data_val.variables['data_site0_var0']
        time_val = netCDF4.num2date(float(NEE_val.getncattr("time_offset")) + np.arange(len(NEE_val[0,:])) * float(NEE_val.getncattr("time_step")), units = NEE_val.getncattr("time_units"))
        time_val = pd.to_datetime(time_val.astype('str'))
        ...
        ## ---------------------
        # get the validation rmse for each site
        rmse_val = data_val.variables['RMSE'][[0,-1],:]
        rmse_val_site = np.concatenate((np.array(rmse_val[0,:][:]), np.array(rmse_val[1,:][:])))
        df_rmse_val_site = pd.DataFrame(rmse[0,:][:])
        df_rmse_val_site = df_rmse_val_site.append(pd.DataFrame(rmse_val[1,:][:]))
        df_rmse_val_site.loc[2] = df_rmse_val_site.iloc[1,:] / df_rmse_val_site.iloc[0,:]
        df_rmse_val_site.columns = ['NEE','GPP','Reco']
        df_rmse_val_site['Site'] = ss
        df_rmse_val_site['Datatype'] = ['Prior','Post','Ratio']
        df_rmse_val_melt = pd.melt(df_rmse_val_site, id_vars=('Site','Datatype'), var_name="Flux", value_name="RMSE")
            
        # appending the data frame with all site RMSE
        df_rmse_val = df_rmse_val.append(df_rmse_val_melt)
...
#        df_param_sub = df_param_sub.append(param_sub_melt)
        ...
        for nf, sf in enumerate(flux_names):     
            ...
            dim_val = data_val.variables[var_name][:].shape
            flux_site_val = data_val.variables[var_name][[0,1], :]
            if dim[0] == 4:
            ...
            ## ---------------------
            #create daily flux data frame for calibration
            ...
            ## ---------------------
            #create daily flux data frame for validation
            flux_site_val = pd.DataFrame(flux_site_val)
            flux_site_val = flux_site_val.transpose()
            flux_site_val.columns = ['Obs','Post']
            flux_site_val['Date'] = time_val
            flux_site_val['Date'] = flux_site_val['Date'].dt.date 
            flux_melt_val = pd.melt(flux_site_val, id_vars='Date')
            flux_melt_val['Site'] = ss
            flux_melt_val['Flux'] = sf
            
            # append the dataframe with new flux data
            df_flux_val = df_flux_val.append(flux_melt_val)
...
#            df_month_mean_unstack = df_month.set_index(['Site','Flux','Month','Datatype'])['Monthly_mean'].unstack().reset_index()
    ...
    # appending the validation RMSE data frame with all optimization and site RMSE
    df_rmse_val['optim'] = i+1
    df_rmse_val['optim_name'] = [j.lower().replace(',', '').split('_iter') for j in validation_name][i][0]
    df_rmse_val_all = df_rmse_val_all.append(df_rmse_val)
...
#            ns = 0
    ...
    ## ---------------------
    ## ---------------------
    # create similar dataframe for daily data (calibration) as monthly dataframes
    ...
    # appending the Correlation coefficient (R) data frame with all optimizations
    flux_daily_corr['optim'] = i+1
    flux_daily_corr['optim_name'] = [j.lower().replace(',', '').split('_iter') for j in optim_name][i][0]
    df_corr_all = df_corr_all.append(flux_daily_corr)
    
    # create similar dataframe for daily data (validation) as above
    df_flux_val_pivot = df_flux_val.set_index(['Date','Site','Flux','variable'])['value'].unstack().reset_index()
    
    flux_daily_val_corr = df_flux_val_pivot.groupby(['Site','Flux'])[['Obs','Post']].corr()
    flux_daily_val_corr.drop(['Post'], axis=1, inplace=True)
    flux_daily_val_corr.drop('Obs', level=2, inplace=True)
    flux_daily_val_corr.reset_index(level=0, inplace=True)
    flux_daily_val_corr.reset_index(level=0, inplace=True)
    flux_daily_val_corr.reset_index(level=0, inplace=True)
    flux_daily_val_corr.columns = ['Simulation','Flux','Site','Daily_corr']
        
    flux_daily_val_corr = pd.merge(df_site_names,flux_daily_val_corr,left_on='Site',right_on='Site',how='outer')
    
    # appending the validation R data frame with all optimization and site RMSE
    flux_daily_val_corr['optim'] = i+1
    flux_daily_val_corr['optim_name'] = [j.lower().replace(',', '').split('_iter') for j in validation_name][i][0]
    df_corr_val_all = df_corr_val_all.append(flux_daily_val_corr)
...
#        nf = 0
        ...
        g.set(xlabel='', ylabel='')
        plt.subplots_adjust(top=0.96,left=0.04,bottom=0.035,right=0.995)
...
#%% Plot RMSE ratio comparison between optimizations and validations
df_rmse_all_sub = df_rmse_all.loc[df_rmse_all['Datatype'] == 'Ratio']
df_rmse_all_sub['sim_type'] = 'Optimization'
df_rmse_all_sub = df_rmse_all_sub.drop('Datatype', 1)
df_rmse_all_sub = df_rmse_all_sub.drop('optim_name', 1)    


df_rmse_val_all_sub = df_rmse_val_all.loc[df_rmse_val_all['Datatype'] == 'Ratio']
df_rmse_val_all_sub['sim_type'] = 'Validation'
df_rmse_val_all_sub = df_rmse_val_all_sub.drop('Datatype', 1)
df_rmse_val_all_sub = df_rmse_val_all_sub.drop('optim_name', 1)    

df_rmse_all_sub = df_rmse_all_sub.append(df_rmse_val_all_sub)

#Box-whisker plots for RMSE    
sns.set(font_scale=1, style='white')  
for nf, sf in enumerate(df_rmse_all_sub.optim.unique()):
#        nf = 0
#    sf = 1
    df_rmse_all_sub_opt = df_rmse_all_sub.loc[df_rmse_all_sub['optim'] == sf]

    g = sns.boxplot(x='Flux', y='RMSE', hue='sim_type', data=df_rmse_all_sub_opt, dodge=True)
    g.figure.set_size_inches(4,4)
    g.set(xlabel='', ylabel='')
            
    plt.subplots_adjust(top=0.92,bottom=0.08,left=0.18,right=0.99)
    if nf==0 or nf==3: 
        plt.text(x=-1.1, y=(max(df_rmse_all_sub_opt.RMSE)+min(df_rmse_all_sub_opt.RMSE))/2, verticalalignment='center', s='Posterior RMSE / Prior RMSE', size=14, rotation=90)
    plt.legend(title='Simulation', bbox_to_anchor=(0.4, 1), ncol=1, fontsize=8)
    plt.suptitle(subplot_title[nf]+' Optimization P'+str(sf)) 
     
    if nf > 0: g.legend_.remove()
        
    plt.savefig(outdir + 'RMSE/RMSE_cal_val_ratio_comparison_P' + str(sf) + '_box_whisker.png')
    plt.close()


df_rmse_all_sub_P1 = df_rmse_all_sub.loc[df_rmse_all_sub['optim'] == 1]
df_rmse_all_sub_P1 = df_rmse_all_sub_P1.drop('optim', 1)
df_rmse_all_sub_P1_pivot = df_rmse_all_sub_P1.set_index(['Site','Flux','sim_type'])['RMSE'].unstack().reset_index()
    
#write the RMSE cdataframe in outputs directory
df_rmse_all_sub_P1_pivot.to_csv(outdir + 'RMSE/' + 'RMSE_cal_val_ratio_comparison.csv', encoding='utf-8', index=False)
    

#Plot RMSE magnitude comparison between optimizations and validations
df_rmse_all_sub = df_rmse_all[df_rmse_all['Datatype'].isin(['Prior','Post'])]
df_rmse_all_sub['sim_type'] = 'Prior'
df_rmse_all_sub.sim_type[df_rmse_all_sub.Datatype == 'Post'] = 'Optimization'
df_rmse_all_sub = df_rmse_all_sub.drop('Datatype', 1)
df_rmse_all_sub = df_rmse_all_sub.drop('optim_name', 1)    


df_rmse_val_all_sub = df_rmse_val_all[df_rmse_val_all['Datatype'].isin(['Post'])]
df_rmse_val_all_sub['sim_type'] = 'Validation'
df_rmse_val_all_sub = df_rmse_val_all_sub.drop('Datatype', 1)
df_rmse_val_all_sub = df_rmse_val_all_sub.drop('optim_name', 1)    
...
##Box-whisker plots for RMSE magnitude   
#sns.set(font_scale=1, style='white')  
#for nf, sf in enumerate(df_rmse_all_sub.optim.unique()):
##        nf = 0
##    sf = 1
...
#    plt.subplots_adjust(top=0.92,bottom=0.08,left=0.18,right=0.99)
#    if nf==0 or nf==3: 
#        plt.text(x=-1.1, y=(max(df_rmse_all_sub_opt.RMSE)+min(df_rmse_all_sub_opt.RMSE))/2, verticalalignment='center', s='RMSE', size=14, rotation=90)
#    plt.legend(title='Simulation', bbox_to_anchor=(0.4, 1), ncol=1, fontsize=8)
#    plt.suptitle(subplot_title[nf]+' Optimization P'+str(sf)) 
#     
#    if nf > 0: g.legend_.remove()
#        
#    plt.savefig(outdir + 'RMSE/RMSE_cal_val_magnitude_comparison_P' + str(sf) + '_box_whisker.png')
#    plt.close()


#Srtip and box plot of RMSE magnitude comparison between optimizations and validations - Only NEE
df_rmse_nee = df_rmse_all_sub[df_rmse_all_sub['Flux'].isin(['NEE'])]

#remove the extra Prior rows
df_rmse_nee_prior = df_rmse_nee[df_rmse_nee['sim_type'].isin(['Prior'])]
df_rmse_nee_prior = df_rmse_nee_prior.drop_duplicates(subset=['Site','sim_type'])
df_rmse_nee_post = df_rmse_nee[df_rmse_nee['sim_type'].isin(['Optimization','Validation'])]
df_rmse_nee = df_rmse_nee_prior.append(df_rmse_nee_post)

g = sns.stripplot(x='optim', y='RMSE', hue='sim_type', data=df_rmse_nee, dodge=True, linewidth=1, edgecolor='gray')
g = sns.boxplot(x='optim', y='RMSE', hue='sim_type', data=df_rmse_nee, dodge=True, fliersize=0)
g.figure.set_size_inches(6,4)
g.set(xlabel='', ylabel='')
            
plt.subplots_adjust(top=0.99,bottom=0.13,left=0.1,right=0.99)
plt.text(x=-1.3, y=(max(df_rmse_nee.RMSE)+min(df_rmse_nee.RMSE))/2, verticalalignment='center', s='NEE RMSE ($gCm^{-2}d^{-1}$)', size=14, rotation=90)
plt.text(x=6, y=max(df_rmse_nee.RMSE), verticalalignment='center', s='(a)', size=14)    
    
handles, labels = g.get_legend_handles_labels()
l = plt.legend(handles[0:3], labels[0:3], bbox_to_anchor=(0.3, 0.9), ncol=1, fontsize=10)
g.set_xticklabels(pd.DataFrame(param_group).param[1:8], rotation=90, fontsize=10)

plt.savefig(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_NEE_box_whisker.png')
plt.close()
    
#Srtip and box plot of RMSE magnitude comparison between optimizations and validations
df_rmse_prior = df_rmse_all_sub[df_rmse_all_sub['sim_type'].isin(['Prior'])]
df_rmse_prior_mean = df_rmse_prior.groupby(['Site','sim_type','Flux'], as_index=False).mean()
df_rmse_prior_mean = df_rmse_prior_mean.drop('optim', 1)

df_optim = pd.DataFrame({'optim':np.repeat(df_rmse_all_sub.optim.unique(),len(site_names)*len(flux_names))})
df_rmse_prior_mean = df_rmse_prior_mean.append([df_rmse_prior_mean]*(len(df_rmse_all_sub.optim.unique())-1),ignore_index=True)
df_rmse_prior_mean = pd.concat([df_rmse_prior_mean,df_optim], axis=1)
df_rmse_post = df_rmse_all_sub[df_rmse_all_sub['sim_type'].isin(['Optimization','Validation'])]
df_rmse_plot = df_rmse_prior_mean.append(df_rmse_post)

for nf, sf in enumerate(df_rmse_all_sub.optim.unique()):
#    nf = 0
#    sf = 1
    df_rmse_plot_sub = df_rmse_plot.loc[df_rmse_plot['optim'] == sf]

    g = sns.stripplot(x='Flux', y='RMSE', hue='sim_type', data=df_rmse_plot_sub, dodge=True, linewidth=1, edgecolor='gray')
    g = sns.boxplot(x='Flux', y='RMSE', hue='sim_type', data=df_rmse_plot_sub, dodge=True, fliersize=0)
    g.figure.set_size_inches(4,4)
    g.set(xlabel='', ylabel='')
            
    plt.subplots_adjust(top=0.92,bottom=0.08,left=0.18,right=0.99)
    if nf==0 or nf==4: 
        plt.text(x=-1.1, y=(max(df_rmse_plot_sub.RMSE)+min(df_rmse_plot_sub.RMSE))/2, verticalalignment='center', s='RMSE', size=14, rotation=90)
    
    handles, labels = g.get_legend_handles_labels()
    l = plt.legend(handles[0:3], labels[0:3], bbox_to_anchor=(0.3, 0.9), ncol=1, fontsize=8)
#    plt.legend(title='Simulation', bbox_to_anchor=(0.4, 0.8), ncol=1, fontsize=8)
    plt.suptitle(subplot_title[nf]+' Optimization P'+str(sf)) 
     
    if nf > 0: g.legend_.remove()
        
    plt.savefig(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P' + str(sf) + '_box_whisker.png')
    plt.close()



#Srtip and box plot of R magnitude comparison between optimizations and validations - Only NEE
df_corr = df_corr_all
df_corr.Simulation[df_corr.Simulation == 'Post'] = 'Optimization'
df_corr_val_all.Simulation = 'Validation'

#df_corr_all.Simulation.unique()
df_corr = df_corr.append(df_corr_val_all)
df_corr_nee = df_corr[df_corr['Flux'].isin(['NEE'])]
df_corr_nee = df_corr_nee.drop('optim_name', 1)
df_corr_nee = df_corr_nee.drop('Flux', 1)

#remove the extra Prior rows
df_corr_nee_prior = df_corr_nee[df_corr_nee['Simulation'].isin(['Prior'])]
df_corr_nee_prior = df_corr_nee_prior.drop_duplicates(subset=['Site','Simulation'])
df_corr_nee_post = df_corr_nee[df_corr_nee['Simulation'].isin(['Optimization','Validation'])]
df_corr_nee = df_corr_nee_prior.append(df_corr_nee_post)

g = sns.stripplot(x='optim', y='Daily_corr', hue='Simulation', data=df_corr_nee, dodge=True, linewidth=1, edgecolor='gray')
g = sns.boxplot(x='optim', y='Daily_corr', hue='Simulation', data=df_corr_nee, dodge=True, fliersize=0)
g.figure.set_size_inches(6,4)
g.set(xlabel='', ylabel='')
            
plt.subplots_adjust(top=0.99,bottom=0.13,left=0.1,right=0.99)
plt.text(x=-1.2, y=(max(df_corr_nee.Daily_corr)+min(df_corr_nee.Daily_corr))/2, verticalalignment='center', s='Correlation (R)', size=14, rotation=90)
plt.text(x=6, y=max(df_corr_nee.Daily_corr), verticalalignment='center', s='(b)', size=14)
    
handles, labels = g.get_legend_handles_labels()
l = plt.legend(handles[0:3], labels[0:3], bbox_to_anchor=(0.15, 0.25), ncol=1, fontsize=10)
g.set_xticklabels(pd.DataFrame(param_group).param[1:8], rotation=90, fontsize=10)

plt.savefig(outdir + 'RMSE/R_cal_val_magnitude_strip_NEE_box_whisker.png')
plt.close()


#Srtip and box plot of R magnitude comparison between optimizations and validations
df_corr = df_corr.drop('optim_name', 1)
df_corr_prior = df_corr[df_corr['Simulation'].isin(['Prior'])]
df_corr_prior_mean = df_corr_prior.groupby(['Site','Simulation','Flux'], as_index=False).mean()
df_corr_prior_mean = df_corr_prior_mean.drop('optim', 1)

df_optim = pd.DataFrame({'optim':np.repeat(df_corr.optim.unique(),len(site_names)*len(flux_names))})
df_corr_prior_mean = df_corr_prior_mean.append([df_corr_prior_mean]*(len(df_corr.optim.unique())-1),ignore_index=True)
df_corr_prior_mean = pd.concat([df_corr_prior_mean,df_optim], axis=1)
df_corr_post = df_corr[df_corr['Simulation'].isin(['Optimization','Validation'])]
df_corr_plot = df_corr_prior_mean.append(df_corr_post)

for nf, sf in enumerate(df_corr.optim.unique()):
#    nf = 0
#    sf = 1
    df_corr_plot_sub = df_corr_plot.loc[df_corr_plot['optim'] == sf]

    g = sns.stripplot(x='Flux', y='Daily_corr', hue='Simulation', data=df_corr_plot_sub, dodge=True, linewidth=1, edgecolor='gray')
    g = sns.boxplot(x='Flux', y='Daily_corr', hue='Simulation', data=df_corr_plot_sub, dodge=True, fliersize=0)
    g.figure.set_size_inches(4,4)
    g.set(xlabel='', ylabel='')
            
    plt.subplots_adjust(top=0.92,bottom=0.08,left=0.16,right=0.99)
    if nf==0 or nf==4: 
        plt.text(x=-1.05, y=(max(df_corr_plot_sub.Daily_corr)+min(df_corr_plot_sub.Daily_corr))/2, verticalalignment='center', s='Correlation (R)', size=14, rotation=90)
    
    handles, labels = g.get_legend_handles_labels()
    l = plt.legend(handles[0:3], labels[0:3], bbox_to_anchor=(0.6, 0.2), ncol=1, fontsize=8)
#    plt.legend(title='Simulation', bbox_to_anchor=(0.4, 0.8), ncol=1, fontsize=8)
    plt.suptitle(subplot_title[nf]+' Optimization P'+str(sf)) 
     
    if nf > 0: g.legend_.remove()
        
    plt.savefig(outdir + 'RMSE/R_cal_val_magnitude_strip_P' + str(sf) + '_box_whisker.png')
    plt.close()
    
    
#%% 
# - load flux corr data for Opt1 P1
flux_corr_nee_p1 = pd.read_csv(outdir + 'Flux_corr/' + 'Flux_corr_optim_' + optim_name[0] + '.csv', sep=",")
flux_corr_nee_p1 = flux_corr_nee_p1.drop('optim', 1)
flux_corr_nee_p1 = flux_corr_nee_p1.drop('optim_name', 1)
...
#        flux_corr_plot.optim_name[flux_corr_plot.Simulation == 'Prior'] = 'Prior'
    ...
    if nf==0: 
        ...
        plt.text(x=-1.3, y=0.25, verticalalignment='center', s='Correlation coefficient (R)', size=14, rotation=90)
...
#%% 
# - load flux corr data for Opt1 P1
flux_slope_nee_p1 = pd.read_csv(outdir + 'Flux_corr/' + 'Flux_slope_optim_' + optim_name[0] + '.csv', sep=",")
...
#        nf = 0
    ...
    if nf==0: 
        plt.subplots_adjust(top=0.93,bottom=0.08,left=0.22,right=0.99)
        plt.text(x=-1.35, y=0.5, verticalalignment='center', s='Slope of linear least-square regression', size=14, rotation=90)
...
df_month_plot_mean['linesize'] = 1
df_month_plot_mean.linesize[df_month_plot_mean.optim == 'Obs'] = 2
sizes = {a:a for a in df_month_plot_mean['linesize'].unique()}

sns.set(font_scale=1.2, style='white')
d = {"ls" : ["--","-","-","-","-","-","-","-","-"], "lw" : [2,1,1,1,1,1,1,1,1]}
g = sns.FacetGrid(df_month_plot_mean, row="site_type", col='Flux', hue="optim", margin_titles=True,
                           ...
                           aspect=1.5, height=3, hue_kws=d)
#g.map_dataframe(sns.lineplot, "Month", "Monthly_mean", size="linesize", sizes=sizes)
g = g.map(plt.plot, "Month", "Monthly_mean", alpha=.7)
[plt.setp(ax.texts, text="") for ax in g.axes.flat]
g.set_titles(row_template = '{row_name}', col_template = '{col_name}')
g.set(xlabel='', ylabel='')
...
#            if nx < len(site_names):
    ...
    plt.legend(bbox_to_anchor=(1.05, 3.75), ncol=3)
    g.set(xticks=df_month_mean.Month.unique()[1::2])
...
if param_plot == True: 
    ...
    ## plotting parameter uncertainty spread for NEE opt with param type
    ## ---------------------
    ## ---------------------
    boxplot_order=[1,2,3,4,5,6,7]
    ...
    df_param_plot['uncer_reduc'] = df_param_plot.groupby(['Param_name','optim','Site']).Paramvalue.transform('median')
    df_param_plot = df_param_plot.drop_duplicates(subset=['Param_name','optim','Site'])
...
#    df_param_plot = pd.merge(df_param_plot, param_list, left_on='Param_name',right_on='Param_name',how='outer')
    ...
    sns.set(font_scale=1, style='white')
    g1 = sns.boxplot(x='optim', y='uncer_reduc', hue='Param_type', data=df_param_plot, dodge=True)  
    g1.set(xlabel='', ylabel='')
#    g1.axvline(0.5, ls='--', c='black') # add a horizontal line at ratio = 1.0
    g1.set_xticklabels(df_param_group.param[1:8], rotation=90, fontsize=10)
    g1.legend_.remove()
#    g1.set(title = 'Sink')
#    g1.set(ylim=(-0.02*max(df_param_plot_nee_uncer.uncer_reduc), 0.02*max(df_param_plot_nee_uncer.uncer_reduc)+max(df_param_plot_nee_uncer.uncer_reduc)))
#    g1.text(x=6.4, y=0.85, verticalalignment='center', s='(a)', size=16)        
#    g1.add_patch(Rectangle((-0.5, -0.015*max(df_param_plot_nee_uncer.uncer_reduc)), 2, 0.03*max(df_param_plot_nee_uncer.uncer_reduc)+max(df_param_plot_nee_uncer.uncer_reduc),linewidth=3,linestyle='dashed',edgecolor='grey',facecolor='none'))
            
    plt.subplots_adjust(top=0.95,bottom=0.08,left=0.1,right=0.99)
#    plt.subplots_adjust(wspace=0.12, hspace=0)
    plt.legend(title='', bbox_to_anchor=(0.2, 0.98), ncol=3, fontsize=10)
    plt.text(x=-1.2, y=50, verticalalignment='center', s='Parameter uncertainty reduction (%)', size=14, rotation=90)        
    
    plt.savefig(outdir + 'Parameters/NEE_param_uncertainty_box_whisker.png', dpi=300)
    plt.close()
...
#    g.fig.suptitle('Parameter uncertainty reduction for all optimizations with all parameters')   
    ...
    g.fig.text(x=0.8, y=0.97, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
...
#    g.set_titles(row_template = '{row_name}')
    ...
    plt.subplots_adjust(top=0.94,left=0.05,bottom=0.62,right=0.96)
...
#    # one ylabel
    ...
    g.fig.text(x=0.05, y=0.96, s='(b)', size=16) 
...
#    g.fig.text(x=0.89, y=0.92, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    ...
    # read param list to add the param types
    param_list = pd.read_excel(param_file)
    df_optim = pd.DataFrame({'optim':np.repeat([1,2,3,4,5,6,7],len(param_list))})
    param_list = param_list.append([param_list]*6,ignore_index=True)
    param_list = pd.concat([param_list,df_optim], axis=1)
...
#    g.fig.suptitle('Parameter deviations for all optimizations with all parameters ( * indicates uncertainty reduction > 50%)')   
    ...
    g.fig.text(x=0.85, y=0.97, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    ...
    ## ---------------------
    ## ---------------------
    df_param_plot_nee = df_param_plot
...
#    g.fig.suptitle('Parameter deviations for NEE optimizations with all parameter sets ( * indicates uncertainty reduction > 50%)')   
    ...
    g.set(xlim=(-0.5, 86.5))
    g.set(xticklabels=[])
    g.fig.text(x=0.05, y=0.94, s='(a)', size=16) 
...
#%%
    # Same plot as above with only parameters having large deviation and uncertainty reduction
    ## ---------------------
    ## ---------------------
    df_param_plot_nee_sub = df_param_plot_nee.loc[(df_param_plot_nee['median_deviation'] > 0.4) | 
                                                  (df_param_plot_nee['median_deviation'] < -0.4) | 
                                                  (df_param_plot_nee['uncer_reduc'] > 50)]
    df_param_plot_nee_sub = df_param_plot_nee_sub.sort_values(['Param_type','Param_name'], ascending=True)
    
#    df_param_plot_nee_sub = df_param_plot_nee.loc[(df_param_plot_nee['uncer_reduc'] > 0.5) | (df_param_plot_nee['Param_name'] == 'NCDGDD_TEMP')]
    
#    df_param_plot_nee_sub = df_param_plot_nee_sub.drop('optim_name', 1)
#    df_param_plot_nee_sub = df_param_plot_nee_sub.drop('asterisk', 1)
#    df_param_plot_nee_sub = df_param_plot_nee_sub.drop('size', 1)
    
#    df_param_plot_nee_sub.loc[(df_param_plot_nee_sub['Param_name'] == 'LEAF_AGE_CRIT_COEFF')]
#    pd.set_option("display.max_rows", None, "display.max_columns", None)

    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim', hue='optim_name', data=df_param_plot_nee_sub,
                    kind='bar', size=1, height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    else: 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim_name', hue='Param_type', data=df_param_plot_nee_sub,
                    kind='bar', size=1, height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    xcoords = [13.5,21.5]
    for xc in xcoords:
        g = g.map(plt.axvline, x=xc, ls='--', c='black') # add vertical lines to seperate parameter groups
#    g = g.map(plt.scatter, x='Param_name', y='asterisk', data=df_param_plot_nee, marker='*', s=2)
#    g = g.map(sns.scatterplot, 'Param_name', 'asterisk', marker='*', size='size', data=df_param_plot_nee_sub)
...
#    g.set_titles(row_template = '{row_name}')
    ...
    plt.subplots_adjust(hspace=0.3, wspace=0.2)
    plt.subplots_adjust(top=0.92,left=0.07,bottom=0.02,right=0.95)
    
    # entire figure title
#    g.fig.suptitle('Parameter deviations for NEE optimizations with all parameter sets ( * indicates uncertainty reduction > 50%)')   
    # one ylabel
    g.fig.text(x=0.005, y=0.5, verticalalignment='center', s='Parameter deviations', size=16, rotation=90)
    g.fig.text(x=0.27, y=0.94, horizontalalignment='center', s='Phenology', size=16, rotation=0)
    g.fig.text(x=0.58, y=0.94, horizontalalignment='center', s='Photosynthesis', size=16, rotation=0)
    g.fig.text(x=0.83, y=0.94, horizontalalignment='center', s='Post C uptake', size=16, rotation=0)
    g.set(ylim=(-0.5, 0.75), yticks=[-0.5,0.5])
#    g.set(xlim=(-0.5, 39.5))
    g.set(xticklabels=[])
    g.fig.text(x=0.05, y=0.94, s='(a)', size=16) 
    ...
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_deviation_with_uncertainty_reduction_NEE_opt_sub.png')
    plt.close()
    

    #plot the same figure as above with only parameters having large deviation and uncertainty reduction
#    df_param_plot_sub = df_param_plot.loc[(df_param_plot['median_deviation'] > 0.4) | 
#                                                  (df_param_plot['median_deviation'] < -0.4) | 
#                                                  (df_param_plot['uncer_reduc'] > 0.5)]
#    
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim', hue='optim_name', data=df_param_plot_nee_sub,
                    kind='bar', size=1, height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
    else:
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim_name', hue='optim_name', data=df_param_plot_nee_sub,
                    kind='bar', size=1, height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
#    xcoords = [35.5,66.5]
    xcoords = [13.5,21.5]
    for xc in xcoords:
...
#    g.set_titles(row_template = '{row_name}')
    ...
    g.fig.set_figheight(4)
    plt.subplots_adjust(hspace=0.65, wspace=0.2)
    plt.subplots_adjust(top=0.92,left=0.07,bottom=0.54,right=0.95)
    
#    # entire figure title
#    g.fig.suptitle('Parameter uncertainty reduction for NEE optimizations with all parameter sets')   
#    # one ylabel
    g.fig.text(x=0.005, y=0.7, verticalalignment='center', s='Uncertainty reduction', size=16, rotation=90)
    g.fig.text(x=0.05, y=0.95, s='(b)', size=16) 
#    g.fig.text(x=0.25, y=0.92, horizontalalignment='center', s='Phenology', size=14, rotation=0)
#    g.fig.text(x=0.64, y=0.92, horizontalalignment='center', s='Photosynthesis', size=14, rotation=0)
#    g.fig.text(x=0.89, y=0.92, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    ...
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_uncertainty_reduction_NEE_opt_sub.png')
    plt.close()
...
#    g.fig.text(x=0.89, y=0.92, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    ...
    ## (NEE opt only) plotting all parameter uncertainty reductions
    #(but only the parameters with non-zero deviations)
    #and only plot parameters for sink site type
    ## ---------------------
    ## ---------------------
    df_param_nee_all_uncer = df_param_all.loc[df_param_all['Paramtype'] == 'uncer_reduc']
    df_param_nee_all_uncer = df_param_nee_all_uncer.drop('Paramtype', 1)
    df_param_nee_all_uncer = df_param_nee_all_uncer.drop('Param_ID', 1)
    
    #assign site type (source / pivot / sink)
    df_param_nee_all_uncer['site_type'] = 'Sink'
#    flux_msd_all.loc[flux_msd_all['Site'] == 'US-SO2', 'site_type'] = 'Pivot'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-Wkg', 'site_type'] = 'Pivot'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-Whs', 'site_type'] = 'Pivot'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-SRG', 'site_type'] = 'Pivot'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-SRM', 'site_type'] = 'Pivot'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-Seg', 'site_type'] = 'Pivot'
    
#    flux_msd_all.loc[flux_msd_all['Site'] == 'US-Seg', 'site_type'] = 'Source'
    df_param_nee_all_uncer.loc[df_param_nee_all_uncer['Site'] == 'US-Aud', 'site_type'] = 'Source'
    df_param_nee_all_uncer = df_param_nee_all_uncer.drop('Site', 1)

    
    df_param_nee_all_uncer['uncer_reduc'] = df_param_nee_all_uncer.groupby(['Param_name','site_type','optim']).Paramvalue.transform('median')
    df_param_nee_all_uncer = df_param_nee_all_uncer.drop_duplicates(subset=['Param_name','site_type','optim'])
    
#    # read param list to add the param types
#    param_list = pd.read_excel(param_file)
#    df_optim = pd.DataFrame({'optim':np.repeat([8,9,10,11,12,13,14],len(param_list))})
#    param_list = param_list.append([param_list]*6,ignore_index=True)
#    param_list = pd.concat([param_list,df_optim], axis=1)
    
    # merge the data frames to have param type
    df_param_nee_all_uncer = pd.merge(df_param_nee_all_uncer, param_list, how='right', on=['Param_name','optim'])
#    df_param_plot = pd.merge(df_param_plot, param_list, left_on='Param_name',right_on='Param_name',how='outer')
    df_param_nee_all_uncer = df_param_nee_all_uncer.sort_values(['Param_type','Param_name'], ascending=True)
    df_param_nee_all_uncer.uncer_reduc = 100*df_param_nee_all_uncer.uncer_reduc
    
    df_param_zt = df_param_nee_all_uncer.copy() # mark the parameters for NEE opt with all param
#    df_param_nee_all_uncer.Param_name.unique()
    
    #only consider NEE opt
    df_param_nee_all_uncer = df_param_nee_all_uncer.loc[df_param_nee_all_uncer['optim'].isin([1,2,3,4,5,6,7])]
    # subset the sink sites only
    df_param_nee_sink_uncer = df_param_nee_all_uncer.loc[df_param_nee_all_uncer['site_type'] == 'Sink']
#    df_param_plot_sub = df_param_plot_sub.drop('site_type', 1)
    
#    df_param_nee_sink_uncer = pd.merge(df_param_nee_sink_uncer, df_param_yt, how='left', on=['Param_name','optim','optim_name','Param_type'])
#    df_param_nee_sink_uncer_sub = (df_param_nee_sink_uncer[~df_param_nee_sink_uncer.median_deviation.isnull()])
    
    #plot parameter deviation bar plot
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim', hue='optim_name', data=df_param_nee_sink_uncer,
                    kind='bar', height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
    else:
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim_name', hue='optim_name', data=df_param_nee_sink_uncer,
                    kind='bar', height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
#    xcoords = [35.5,66.5]
    xcoords = [34.5,64.5]
    for xc in xcoords:
...
#    g.set_titles(row_template = '{row_name}')
    ...
    g.fig.set_figheight(4)
    plt.subplots_adjust(hspace=0.8, wspace=0.2)
    plt.subplots_adjust(top=0.94,left=0.05,bottom=0.6,right=0.96)
    
#    # one ylabel
    g.fig.text(x=0.005, y=0.6, verticalalignment='center', s='Parameter uncertainty reduction', size=14, rotation=90)
    g.fig.text(x=0.05, y=0.96, s='(b)', size=14) 
    
    for nx, ax in enumerate(g.axes.flat):
        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=9) # set_xticklabels
        # This contains the right ylabel text
        txt = ax.texts[0]
        ax.text(txt.get_unitless_position()[0], txt.get_unitless_position()[1],
                df_param_group.param[nx+1],transform=ax.transAxes,va='center',fontsize='large')
    
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_uncertainty_reduction_NEE_opt_sinksite.png')
    plt.close()
    
    
    # subset the pivot sites only
    df_param_nee_pivot_uncer = df_param_nee_all_uncer.loc[df_param_nee_all_uncer['site_type'] == 'Pivot']
#    df_param_plot_sub = df_param_plot_sub.drop('site_type', 1)
...
#    df_param_nee_sink_uncer_sub = (df_param_nee_sink_uncer[~df_param_nee_sink_uncer.median_deviation.isnull()])
    ...
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim', hue='optim_name', data=df_param_nee_pivot_uncer,
                    kind='bar', height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
    else:
        g = sns.catplot(x='Param_name', y='uncer_reduc', row='optim_name', hue='optim_name', data=df_param_nee_pivot_uncer,
                    kind='bar', height=0.5, aspect=8, palette=['grey'], margin_titles=True, legend=False, dodge=False) 
#    xcoords = [35.5,66.5]
    xcoords = [35.5,65.5]
    for xc in xcoords:
...
#    # one ylabel
    ...
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_uncertainty_reduction_NEE_opt_pivotsite.png')
    plt.close()
    ...
    ## ---------------------
    ## ---------------------
    df_param_plot_sub = (df_param_plot[~df_param_plot.median_deviation.isnull()])
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim', hue='optim_name', data=df_param_plot_sub,
                    kind='bar', size=3, aspect=4, margin_titles=True, legend=False, dodge=False) 
    else: 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim_name', hue='Param_type', data=df_param_plot_sub,
                    row_order=boxplot_order, kind='bar', height=3, aspect=4, margin_titles=True, legend=False, dodge=False) 
    xcoords = [35.5,65.5]
    for xc in xcoords:
        g = g.map(plt.axvline, x=xc, ls='--', c='black') # add vertical lines to seperate parameter groups
#    g = g.map(plt.scatter, x='Param_name', y='asterisk', data=df_param_plot, marker='*', s=2)
    g.map(sns.scatterplot, 'Param_name', 'asterisk', marker='*', size='size', data=df_param_plot_sub)
...
#    g.set_titles(row_template = '{row_name}')
    ...
    g.fig.text(x=0.875, y=0.97, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    g.set(ylim=(-0.5, 0.75), yticks=[-0.5,0.5])
    g.set(xlim=(-0.5, 80.5))
    ...
    ## ---------------------
    ## ---------------------
    df_param_plot_nee_dev = (df_param_plot_nee[~df_param_plot_nee.median_deviation.isnull()])
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim', hue='optim_name', data=df_param_plot_nee_dev,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    else: 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim_name', hue='Param_type', data=df_param_plot_nee_dev,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    xcoords = [35.5,65.5]
    for xc in xcoords:
        g = g.map(plt.axvline, x=xc, ls='--', c='black') # add vertical lines to seperate parameter groups
#    g = g.map(plt.scatter, x='Param_name', y='asterisk', data=df_param_plot_nee_dev, marker='*', s=2)
    g = g.map(sns.scatterplot, 'Param_name', 'asterisk', marker='*', size='size', data=df_param_plot_nee_dev)
...
#    g.fig.suptitle('Parameter deviations for NEE optimizations with all parameter sets ( * indicates uncertainty reduction > 50%)')   
    ...
    g.fig.text(x=0.9, y=0.94, horizontalalignment='center', s='Post C uptake', size=14, rotation=0)
    g.set(ylim=(-0.5, 0.75), yticks=[-0.5,0.5])
    g.set(xlim=(-0.5, 80.5))
    g.set(xticklabels=[])
...
#        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=7) # set_xticklabels
    ...
    df_param_nee_all_devi = df_param_all.loc[df_param_all['Paramtype'] == 'deviation']
    df_param_nee_all_devi = df_param_nee_all_devi.drop('Paramtype', 1)
    df_param_nee_all_devi = df_param_nee_all_devi.drop('Param_ID', 1)
    
    
    #assign site type (source / pivot / sink)
    df_param_nee_all_devi['site_type'] = 'Sink'
#    flux_msd_all.loc[flux_msd_all['Site'] == 'US-SO2', 'site_type'] = 'Pivot'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-Wkg', 'site_type'] = 'Pivot'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-Whs', 'site_type'] = 'Pivot'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-SRG', 'site_type'] = 'Pivot'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-SRM', 'site_type'] = 'Pivot'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-Seg', 'site_type'] = 'Pivot'
    
#    flux_msd_all.loc[flux_msd_all['Site'] == 'US-Seg', 'site_type'] = 'Source'
    df_param_nee_all_devi.loc[df_param_nee_all_devi['Site'] == 'US-Aud', 'site_type'] = 'Source'
    df_param_nee_all_devi = df_param_nee_all_devi.drop('Site', 1)
    
    df_param_nee_all_devi['median_deviation'] = df_param_nee_all_devi.groupby(['Param_name','site_type','optim']).Paramvalue.transform('median')
    df_param_nee_all_devi = df_param_nee_all_devi.drop_duplicates(subset='median_deviation')
...
#    param_list = pd.concat([param_list,df_optim], axis=1)
    ...
    # merge the data frames to have param type
    df_param_nee_all_devi = pd.merge(df_param_nee_all_devi, param_list, how='right', on=['Param_name','optim'])
    df_param_nee_all_devi = df_param_nee_all_devi.sort_values(['optim','Param_type','Param_name'], ascending=True)

    df_param_nee_all_devi = df_param_nee_all_devi.drop('Paramvalue', 1)
    
#    df_param_zt = df_param_zt.drop('site_type', 1)
#    df_param_zt = df_param_zt.drop('median_deviation', 1)
    df_param_zt = df_param_zt.drop('Paramvalue', 1)
    
    df_param_nee_all_devi = pd.merge(df_param_nee_all_devi, df_param_zt, how='left', on=['Param_name','optim','optim_name','Param_type','site_type'])
    df_param_nee_all_devi = df_param_nee_all_devi.drop_duplicates(subset=['Param_name','site_type','optim'])
    
    threshold = 50
    df_param_nee_all_devi['asterisk'] = 1
    df_param_nee_all_devi.loc[(df_param_nee_all_devi['uncer_reduc'] > threshold), 'asterisk'] = 0.65

    df_param_nee_all_devi['size'] = 0
    df_param_nee_all_devi.loc[(df_param_nee_all_devi['asterisk'] == 0.65),'size'] = 2
    
    #only consider NEE opt
    df_param_nee_all_devi = df_param_nee_all_devi.loc[df_param_nee_all_devi['optim'].isin([1,2,3,4,5,6,7])]
    #subset the sink sites
    df_param_nee_sink_devi = df_param_nee_all_devi.loc[df_param_nee_all_devi['site_type'] == 'Sink']
    
#    df_param_nee_sink_devi_sub = (df_param_nee_sink_devi[~df_param_nee_sink_devi.median_deviation.isnull()])
    
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim', hue='optim_name', data=df_param_nee_sink_devi,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    else: 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim_name', hue='Param_type', data=df_param_nee_sink_devi,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    xcoords = [34.5,64.5]
    for xc in xcoords:
        g = g.map(plt.axvline, x=xc, ls='--', c='black') # add vertical lines to seperate parameter groups
#    g = g.map(plt.scatter, x='Param_name', y='asterisk', data=df_param_plot_nee, marker='*', s=2)
#    g = g.map(sns.scatterplot, 'Param_name', 'asterisk', marker='*', size='size', data=df_param_nee_sink_devi)
...
#    g.set(xlim=(-0.5, 80.5))
    g.set(xticklabels=[])
    g.fig.text(x=0.05, y=0.94, s='(a)', size=14) 
...
#        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=7) # set_xticklabels
    ...
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_deviation_with_uncertainty_reduction_NEE_opt_sinksite.png')
    plt.close()    
    
    
    #subset the pivot sites
    df_param_nee_pivot_devi = df_param_nee_all_devi.loc[df_param_nee_all_devi['site_type'] == 'Pivot']
    
#    df_param_nee_sink_devi_sub = (df_param_nee_sink_devi[~df_param_nee_sink_devi.median_deviation.isnull()])
    
    sns.set(font_scale=1, style='white')
    if any('paramP7' in s for s in optim_name): 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim', hue='optim_name', data=df_param_nee_pivot_devi,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    else: 
        g = sns.catplot(x='Param_name', y='median_deviation', row='optim_name', hue='Param_type', data=df_param_nee_pivot_devi,
                    kind='bar', height=0.5, aspect=8, margin_titles=True, legend=False, dodge=False) 
    xcoords = [35.5,65.5]
    for xc in xcoords:
        g = g.map(plt.axvline, x=xc, ls='--', c='black') # add vertical lines to seperate parameter groups
...
#        plt.setp(ax.get_xticklabels(), rotation=90, fontsize=7) # set_xticklabels
    ...
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_deviation_with_uncertainty_reduction_NEE_opt_pivotsite.png')
    plt.close()   
...
#    g.set_titles(row_template = '{row_name}')
    ...
    g.fig.text(x=0.90, y=0.94, horizontalalignment='center', s='Post C uptake', size=12, rotation=0)
...
#%% Plot dominant PFT-wise parameter posterior spread (NEE opt P1 only)
    df_param_p1 = df_param_all.loc[df_param_all['optim'] == 1]
    df_param_p1 = df_param_p1.loc[df_param_p1['Paramtype'].isin(['Param_post', 'uncer_reduc', 'deviation'])]
    df_param_p1 = df_param_p1.drop('Param_ID', 1)
    df_param_p1 = df_param_p1.drop('optim_name', 1)
    df_param_p1 = df_param_p1.drop('optim', 1)
    df_param_p1 = df_param_p1.drop_duplicates(subset=['Site','Param_name','Paramtype'])
    
    # merge the data frames to have dominant PFT types
    pft_type = np.loadtxt(site_file, dtype=str)[:,:]
    pft_type = pd.DataFrame(pft_type)
    pft_type = pft_type.drop(1, 1)
    pft_type = pft_type.rename(columns={0: 'Site', 2:'pft'})
    
    df_param_p1 = pd.merge(df_param_p1, pft_type, how='right', on='Site')
#    df_param_p1 = df_param_p1.drop('Site', 1)
    
#    df_param_p1['Param_median'] = df_param_p1.groupby(['Param_name','Paramtype']).Paramvalue.transform('median')
#    df_param_p1 = df_param_p1.drop('Paramvalue', 1)
#    df_param_p1 = df_param_p1.drop_duplicates(subset=['Param_name','Paramtype','Site'])
    
    df_param_p1_pivot = df_param_p1.set_index(['Site','pft','Param_name','Paramtype'])['Paramvalue'].unstack().reset_index()
    
#    df_param_p1_pivot.loc[(df_param_p1_pivot['Param_name'] == 'RESIDENCE_TIME')]
#    df_param_p1_pivot.loc[(df_param_p1_pivot['Param_name'] == 'BRJV')]
    
    df_param_p1_thresh = df_param_p1_pivot.loc[(df_param_p1_pivot['deviation'] > threshold) | (df_param_p1_pivot['deviation'] < -threshold) | (df_param_p1_pivot['uncer_reduc'] >= 0.5)]
    df_param_p1_sub = df_param_p1_pivot.loc[df_param_p1_pivot['Param_name'].isin(df_param_p1_thresh['Param_name'].unique())]
    
    df_param_p1_sub = df_param_p1_sub.drop('deviation', 1)
    df_param_p1_sub = df_param_p1_sub.drop('uncer_reduc', 1)
    
    pft_type = pft_type.append([pft_type]*(len(df_param_p1_sub.Param_name.unique())-1),ignore_index=True)
    
    param_list_sub = pd.DataFrame({'Param_name':np.repeat(df_param_p1_sub.Param_name.unique(),len(site_names))})
    param_list_sub = pd.concat([pft_type,param_list_sub], axis=1)
    
    df_param_p1_merge = pd.merge(df_param_p1_sub, param_list_sub, how='right', on=['Site','pft','Param_name'])
    
    # merge the data frames to have param type
    param_list = pd.read_excel(param_file)
    df_param_p1_merge = pd.merge(df_param_p1_merge, param_list, how='left', on=['Param_name'])
    
#    df_param_p1_sub = df_param_p1_sub.sort_values(['Param_name'], ascending=True)
    df_param_p1_merge.pft = df_param_p1_merge.pft.astype("category")
    df_param_p1_merge.pft.cat.set_categories(['ENT','Shr','C4G'], inplace=True)
    df_param_p1_merge = df_param_p1_merge.sort_values(['Param_type','Param_name','pft'])
    
    df_param_p1_merge.replace(np.nan, np.inf, inplace=True)
#    df_param_p1_merge.dtypes
#    df_param_p1_merge['Param_type'].astype(str)
#    df_param_p1_merge['Param_name'].astype(str)
    df_param_p1_merge['Param'] = df_param_p1_merge['Param_type'] + ' \n ' + df_param_p1_merge['Param_name']
    
#    df_param_p1_merge.loc[(df_param_p1_merge['Param_name'] == 'BRJV')]
    
    sns.set(font_scale=0.8, style='white')
    g = sns.relplot(x='pft', y='Param_post', hue='pft', style='pft', 
                col='Param', palette=['green','grey','orange'], 
                col_wrap=9, data=df_param_p1_merge, 
                height=5, aspect=1, s=70, sizes=(10,50),
...
#    plt.xlim(-0.5, 2.5)
    ...
    g.set_titles(col_template = '{col_name}')
    g.set(xlabel='', ylabel='')
    plt.subplots_adjust(top=0.95,left=0.035,bottom=0.05,right=0.96)
...
#        g.fig.suptitle('Parameters ' + r"$\bf{FOR}$ " + ' site ' + site_names[ns] + r" $\bf{WITH}$ " + 'all optimizations')   
    ...
    g.set(xticks=list(range(0,3)))
    g.set_xticklabels(df_param_p1_merge.pft.unique(), rotation=90, fontsize=8)
            
    g.fig.savefig(outdir + 'Parameters/' + 'Parameters_NEE_P1_pft_wise.png', dpi = 1000)
    plt.close()
    

#%% Plot parameter posterior spread with TRY field data
    df_param_try_fuf = df_param_all.loc[(df_param_all['Param_name'] == 'SLA') & (df_param_all['Site'] == 'US-Fuf')]
    df_param_try_ses = df_param_all.loc[(df_param_all['Param_name'].isin(['VCMAX25', 'LEAFLIFE_TAB'])) & (df_param_all['Site'] == 'US-Ses')]
    
    df_param_try = pd.concat([df_param_try_fuf,df_param_try_ses], ignore_index=True)
    
#    df_param_try = df_param_all.loc[df_param_all['Param_name'].isin(['SLA', 'VCMAX25', 'LEAFLIFE_TAB'])]
#    df_param_try = df_param_try.loc[df_param_try['Site'].isin(['US-Fuf', 'US-Ses'])]
    df_param_try = df_param_try.loc[df_param_try['Paramtype'].isin(['Param_post'])]
    df_param_try = df_param_try.drop('optim_name', 1)
    df_param_try = df_param_try.drop('Param_ID', 1)
    df_param_try = df_param_try.drop('Paramtype', 1)
    df_param_try['datatype'] = 'Modelled'

#    Read the TRY data
    TRY_data = pd.read_csv(rundir + 'TRY_param_data.csv', sep=",")
    TRY_data = TRY_data.drop('Species', 1)
    TRY_data['datatype'] = 'Observation'
    TRY_data['optim'] = 0

    df_param_try = pd.concat([df_param_try,TRY_data], ignore_index=True)
    
    sns.set(font_scale=1, style='white')
    g = sns.catplot(x="datatype", y="Paramvalue", hue="datatype", col="Param_name", data=df_param_try, kind="strip", height=4, aspect=.7,
                    s = 15, margin_titles=True, legend=False, sharey=False)
    [plt.setp(ax.texts, text="") for ax in g.axes.flat]
    g.set_titles(col_template = '{col_name}')                
#    g = sns.stripplot(x='datatype', y='Paramvalue', hue='datatype', col='Param_name', data=df_param_try, dodge=True, linewidth=1, edgecolor='gray')
#    g = sns.boxplot(x='Flux', y='Daily_corr', hue='Simulation', data=df_corr_plot_sub, dodge=True, fliersize=0)
    g.set(xlabel='', ylabel='')
            
    plt.subplots_adjust(top=0.92,bottom=0.14,left=0.1,right=0.99)
    plt.text(x=-6.3, y=50, s='Parameter value', size=14, rotation=90)
    plt.text(x=-4.9, y=15, s='US-Fuf', size=12, rotation=0)
    plt.text(x=-2.2, y=15, s='US-Ses', size=12, rotation=0)
    plt.text(x=0.2, y=15, s='US-Ses', size=12, rotation=0)
    
#    plt.legend(title='Simulation', bbox_to_anchor=(0.4, 0.8), ncol=1, fontsize=8)
#    plt.suptitle(subplot_title[nf]+' Optimization P'+str(sf)) 
        
    plt.savefig(outdir + 'Parameters/Param_modelled_TRY_strip_plot.png')
    plt.close()


#%% 
# reorganise parameter dataframe    
...
## ---------------------
if corr_plot == True: 
    boxplot_order=[0,1,2,3,4,5,6,7]
    boxplot_order_sub=[0,1,2,3,4,5,6,7]
...
#        g1.set(title = subplot_title[nf]+' '+sf)
        ...
        if sf == 'NEE':
            ...
            g1.text(x=1.1, y=80000, verticalalignment='center', s='(a)', size=12)        
#        else: g1.set(ylim=(-10000,390000))
        if sf == 'GPP': g1.text(x=1.1, y=155000, verticalalignment='center', s='(b)', size=12)        
        if sf == 'Reco': g1.text(x=1.1, y=340000, verticalalignment='center', s='(c)', size=12)        
...
#            g2.set(ylim=(-0.03*max(msd_annual_melt_p1.value), 0.03*max(msd_annual_melt_p1.value)+max(msd_annual_melt_p1.value)))
            g2.set(ylim=(-50, 2800))
            g2.text(x=1.1, y=2500, verticalalignment='center', s='(d)', size=12)        
...
#        else: g3.set(ylim=(-10000,390000))
        if sf == 'GPP': g3.text(x=1.1, y=14000, verticalalignment='center', s='(h)', size=12)        
        if sf == 'Reco': g3.text(x=1.1, y=4200, verticalalignment='center', s='(i)', size=12)        
...
#            plt.suptitle(sf+' Flux')   
    ...
    flux_corr_all_plot.optim_name[flux_corr_all_plot.Simulation == 'Prior'] = 'Prior'
    flux_corr_all_plot = pd.merge(flux_corr_all_plot,df_param_group,left_on='optim',right_on='optim',how='outer')
    flux_corr_all_plot = flux_corr_all_plot.loc[flux_corr_all_plot['optim'].isin([0,1])]
    ...
    flux_slope_all_plot.optim_name[flux_slope_all_plot.Simulation == 'Prior'] = 'Prior'   
    flux_slope_all_plot = pd.merge(flux_slope_all_plot,df_param_group,left_on='optim',right_on='optim',how='outer')
    flux_slope_all_plot = flux_slope_all_plot.loc[flux_slope_all_plot['optim'].isin([0,1])]
...
#    df_annual_all_plot = df_annual_all_plot[df_annual_all_plot['optim_name'].isin(['Prior','nee'])]
    df_annual_all_plot = df_annual_all_plot[df_annual_all_plot['optim'].isin([0,1])]
    df_annual_all_plot = df_annual_all_plot.drop_duplicates(subset=['Year','Site','Flux','optim','optim_name','Simulation','param'], keep="first")
...
if rmse_plot == True: 
    ...
    df_rmse_all_plot = df_rmse_all_plot.loc[df_rmse_all_plot['optim'].isin([1])]
    ...
    #write the dataframe in outputs directory
    df_rmse_pivot.to_csv(outdir + 'RMSE/' + 'Flux_rmse_optim_' + optim_name[0] + '.csv', encoding='utf-8', index=False)
...
#    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_2.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_3.png', dpi = 3000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_4.png', dpi = 3000, bbox_inches='tight')
    plt.close()
    ...
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'Parameters/Parameters_deviation_with_uncertainty_reduction_NEE_opt_sub.png')
    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_sub.png')
...
#    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_subset.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_5.png', dpi = 300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S1.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S2.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    ...
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_NEE_box_whisker.png')
    img2 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_NEE_box_whisker.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S3.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    
    
    img1 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P1_box_whisker.png')
    img2 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P2_box_whisker.png')
    img3 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P3_box_whisker.png')
    img4 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P4_box_whisker.png')
    img5 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P5_box_whisker.png')
    img6 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P6_box_whisker.png')
    img7 = mpimg.imread(outdir + 'RMSE/RMSE_cal_val_magnitude_strip_P7_box_whisker.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S3_1.png', dpi = 1000, bbox_inches='tight')
    plt.close()

    
    img1 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P1_box_whisker.png')
    img2 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P2_box_whisker.png')
    img3 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P3_box_whisker.png')
    img4 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P4_box_whisker.png')
    img5 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P5_box_whisker.png')
    img6 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P6_box_whisker.png')
    img7 = mpimg.imread(outdir + 'RMSE/R_cal_val_magnitude_strip_P7_box_whisker.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S3_2.png', dpi = 1000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S4.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S5.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S6.png', dpi = 300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S7.png', dpi = 1000, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S8.png', dpi = 300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S9.png', dpi = 1000, bbox_inches='tight')
    plt.close()
...
#    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_subset.png')
    ...
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'Parameters/NEE_param_uncertainty_box_whisker.png')
    gs = gridspec.GridSpec(1, 1)
    gs.update(wspace=0.005, hspace=0)
    ax1 = plt.subplot(gs[0, 0])
    ax1 = plt.imshow(img1)
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S11.png', dpi = 1000, bbox_inches='tight')
    plt.close()
       
    
    ## plotting manuscript figure #S12
    ## ---------------------
    ## ---------------------     
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S12.png', dpi = 2000, bbox_inches='tight')
    plt.close()
    
    
    ## plotting manuscript figure #S13
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'Parameters/Param_modelled_TRY_strip_plot.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S13.png', dpi = 1500, bbox_inches='tight')
    plt.close()
    
    
        
    ## plotting manuscript figure #S14
    ## ---------------------
    ## ---------------------     
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S14.png', dpi = 1500, bbox_inches='tight')
    plt.close()
    
    
    ## plotting manuscript figure #S15
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'Parameters/Parameters_deviation_with_uncertainty_reduction_NEE_opt_sinksite.png')
    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_sinksite.png')
#    img1 = mpimg.imread(outdir + 'Parameters/Parameters_deviation_with_uncertainty_reduction_NEE_opt_subset.png')
#    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_subset.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S15.png', dpi = 1000, bbox_inches='tight')
    plt.close()


#    ## plotting manuscript figure #S14_2
#    ## ---------------------
#    ## ---------------------     
#    img1 = mpimg.imread(outdir + 'Parameters/Parameters_deviation_with_uncertainty_reduction_NEE_opt_pivotsite.png')
#    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_pivotsite.png')
##    img1 = mpimg.imread(outdir + 'Parameters/Parameters_deviation_with_uncertainty_reduction_NEE_opt_subset.png')
##    img2 = mpimg.imread(outdir + 'Parameters/Parameters_uncertainty_reduction_NEE_opt_subset.png')
#    
#    gs = gridspec.GridSpec(2, 1)
...
#    plt.axis('off')
#        
#    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S14_2_pivotsite.png', dpi = 600, bbox_inches='tight')
#    plt.close()        
    
    
    ## plotting manuscript figure #S16
    ## ---------------------
    ## ---------------------     
    img1 = mpimg.imread(outdir + 'Parameters/Parameters_NEE_P1_pft_wise.png')
    ...
    plt.savefig(outdir + 'Manuscript_figures/' + 'figure_S16.png', dpi = 1500, bbox_inches='tight')
    plt.close()
